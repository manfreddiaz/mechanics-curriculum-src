...
class MetaTrainingEnvironment(gym.Env[Agent, int]):
    ...
    def step(self, action: int) -> Tuple[Agent, float, bool, dict]:

        info = dict()

        trainer_action, evaluator_action = action

        # if envs use the `AutoResetWrapper`, it doesn't matter
        # if the episode (interaction with nature) ends in the middle
        # of a play, `dones` in the RBs capture termination
        # and so do the algorithms? DQN does it.
